...
setupWebViewJavascriptBridge(function(bridge) {
    ...
    $(function(){
	    ...
	    //消息页面跳转便民服务
     	...
     	$('#turn_czb').click(function(){
            bridge.callHandler('pushViewController',{'type':100,'url':'http://m.mychebao.com/czhib_promote/temFriIndex.htm?id=378'});
        });
...
//    						bridge.callHandler('pushViewController',{'type':100,'url':'http://m.banmachewu.com?tn=bjjj'});
//    					});
    				} else if (msg.count >= 2) {
    					/* 活跃用户 */
    					...
    					/* 办理须知上侧广告 */
    					$("#adDiv").removeClass("none")
    					$('#adImg').click(function(){
    						bridge.callHandler('pushViewController',{'type':100,'url':'http://insurert.union-ins.cn/product/wechat/insure/847685440834260991/traffic.html?channelId=188'});
    					});
    				}
    			}
    		},
						...
						if(carsize > 0){
							...
							for(var i = 0; i < carsize ; i++){
								...
								if(carinfo.applyflag == '1'){//可以申请
									carhtml += '<span class="right shenqing" onclick="shenqing(\''+carinfo.applyid+'\',\''+carinfo.carid+'\',\''+userid+'\',\''+carinfo.licenseno+'\',\''+carinfo.cartype+'\')">+申请</span>';
								}
								carhtml += '</div>';
...
function shenqing(applyid,carid,userid,licenseno,cartype){
	$("#apply2").val(applyid);
	$("#carid").val(carid);
	$("#userid").val(userid);
